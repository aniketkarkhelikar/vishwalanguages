    ...
    <motion.div variants={pageSlideUp} initial="hidden" animate="visible" exit="exit" className="bg-paper">
      ...
      <section className="relative min-h-[95vh] flex flex-col justify-end pb-12 md:pb-24 pt-40 px-6 overflow-hidden" style={{ backgroundColor: hc.primary }}>
        ...
        <div className="absolute inset-0 z-0 pointer-events-none flex items-center justify-center">
          <div className="w-[120vw] h-[120vw] md:w-[60vw] md:h-[60vw] rounded-full blur-[120px] opacity-[0.25] mix-blend-overlay animate-glow" style={{ background: `radial-gradient(circle, ${hc.accent} 0%, transparent 70%)` }} />
      ...
      <section className="py-24 md:py-32 bg-surface rounded-t-[3rem] md:rounded-t-[5rem] -mt-8 relative z-20">
        <div className="container-site max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-12 gap-6">
            {/* Phase 1 & 2 - Top Row */}
            <motion.div variants={fadeUpScale} initial="hidden" whileInView="visible" viewport={{ once: true }} className="md:col-span-7 bg-white rounded-3xl p-10 md:p-12 shadow-sm border border-ink/5">
              <span className="text-[10px] font-mono uppercase tracking-widest mb-6 block text-ink/30">Phase 01 — {d.roadmap[0].duration}</span>
              <h3 className="font-display text-3xl md:text-4xl text-ink leading-tight mb-4">{d.roadmap[0].title}</h3>
              <p className="text-ink/60 font-light text-lg leading-relaxed">{d.roadmap[0].desc}</p>
            </motion.div>
            ...
            <motion.div variants={fadeUpScale} initial="hidden" whileInView="visible" viewport={{ once: true }} className="md:col-span-12 rounded-3xl p-10 md:p-16 relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-12" style={{ backgroundColor: hc.primary }}>
              ...
              <div className="relative z-10 w-24 h-24 rounded-full bg-white/10 border border-white/20 flex items-center justify-center shrink-0">
            ...
            <motion.div variants={fadeUpScale} initial="hidden" whileInView="visible" viewport={{ once: true }} transition={{ delay: 0.1 }} className="md:col-span-7 bg-white rounded-3xl p-10 md:p-12 shadow-sm border border-ink/5">
              <span className="text-[10px] font-mono uppercase tracking-widest mb-6 block text-ink/30">Phase 05 — {d.roadmap[4].duration}</span>
              <h3 className="font-display text-3xl md:text-4xl text-ink leading-tight mb-4">{d.roadmap[4].title}</h3>
              <p className="text-ink/60 font-light text-lg leading-relaxed">{d.roadmap[4].desc}</p>
            </motion.div>
          </div>
        </div>
      </section>
      ...
      {/* ══ GRAND CTA ══ */}
      <section className="py-32 md:py-48 text-center bg-surface border-t border-ink/5">
        <div className="container-site">
          <span className="text-[10px] font-mono uppercase tracking-widest block mb-8" style={{ color: hc.primary }}>Begin</span>
          <h2 className="font-display text-5xl md:text-7xl lg:text-8xl leading-none tracking-tight text-ink mb-12">
